...
use App\Http\Requests\Api\Siswa\UpdateFotoProfilRequest;
...
class SiswaController extends Controller
{
    ...
    public function statistik(Request $request)
    {
        try {
            ...
            $rataRata = HasilKuis::where('siswa_id', $siswa->id)->avg('nilai');
            $rataRata = $rataRata ? round((float) $rataRata, 2) : 0;

            // Total chat ke AI
            $totalChat = ChatHistory::where('siswa_id', $siswa->id)->count();

            return response()->json([
                'message' => 'Statistik siswa berhasil dimuat.',
                'data' => [
                    'profil' => new UserResource($siswa),
                    'statistik' => [
            ...
            Log::error('Error saat memuat statistik siswa: ' . $e->getMessage());
            return response()->json([
                'message' => 'Gagal memuat statistik siswa.',
    ...
    public function pertemuanAktif(Request $request)
    {
        try {
            ...
            Log::error('Error saat memuat pertemuan aktif: ' . $e->getMessage());
            return response()->json([
                'message' => 'Gagal memuat pertemuan aktif.',
                'error' => config('app.debug') ? $e->getMessage() : null
            ], 500);
        }
    }
    ...
    public function updateFotoProfil(UpdateFotoProfilRequest $request)
    {
        try {
            $siswa = $request->user();
            ...
            Log::error('Error saat memperbarui foto profil: ' . $e->getMessage());
            return response()->json([
